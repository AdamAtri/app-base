...
export class ThreadListView extends GridLayout {
  ...
  public get listview():ListView { 
    if (! this._listview) {
      this._listview = factory.makeListView({
        ...
        // ]
        on: [{name: ListView.unloadedEvent, callback: (e) => {debugger;} }]
  ...
  public get items():Array<Thread> { return this._items; }
  public set items(v:Array<Thread>) {
    if (this._items === v) { return; }
    this._items = this.listview.items = v;
  }

  protected _itemTemplate:Template = () => new ThreadItemView();
  public get itemTemplate():Template { return this._itemTemplate; }
  public set itemTemplate(v:Template) {
    if (this._itemTemplate === v) { return; }
    this._itemTemplate = this.listview.itemTemplate = v;
  }

  protected _onItemTapped(e:ItemEventData):void {
    trace.write("item-tap: " + JSON.stringify(e.view.bindingContext), trace.categories.Debug);
  }

  protected _onLoadMoreItems():void {
    trace.write("loadMoreItems: " + (this.listview ? this.listview.items.length : 0), trace.categories.Debug);
  }
}